...
public class RecommendationService
{
    ...
    public List<Book> GetRecommendations(string account)
    {
        var allRatings = _ratingRepository.GetAllRatings();
        var targetRatings = allRatings.Where(r => r.MemberID == account).ToList();
        ...
        {
            if (member.Account == account) continue;

            var otherRatings = allRatings.Where(r => r.MemberID == member.Account).ToList();
        ...
        return allRatings
            .Where(r => r.MemberID == bestMatch.Account && !ratedIsbns.Contains(r.Isbn))
